...
import { ReverseData } from "@/utils/types/payment";

// apis
import { paymentReverse } from "@/apis/payment";

function page() {
  const [navigatePath, setNavigatePath] = useState("");
  const [isSent, setIsSent] = useState(false);
  const [isFetching, setIsFetching] = useState(true);
  const [input, setInput] = useState(REVERSE_API.sample);
  ...
  const varidateData = (data: any) => {
    if (data instanceof Object) {
      const saleData: ReverseData = data as ReverseData;
      return saleData;
    } else {
      console.log("Data is not of type ReverseData");
    }
  };

  const postData = async (data: ReverseData) => {
    setIsFetching(true);
    setNavigatePath(`/transaction?requestId=${data?.requestId}`);
    const res: any = await paymentReverse(data);
    setIsSent(true);
    setIsFetching(false);
  ...
  return (
    <div className=" relative sm:ms-64 translate-y-16 mx-2 grid grid-cols-12 gap-2 ">
      <div className="col-span-12 sm:col-span-7 ms-2 space-y-4">
        <h1 className=" mt-6 text-2xl font-extrabold">Payment Reverse API</h1>
        <hr />

        {REVERSE_API_DOC.map((attr, idx) => (
          <div key={idx}>
      ...
      <div className="col-span-12 sm:col-span-5 space-y-3 bg-gray-100 p-3 rounded-md border-2 border-gray-400 sm:my-3 my-6">
        ...
        <Endpoint link={REVERSE_API.endpoint} method={REVERSE_API.method} />
        <BodyInput rows={4} title={"Request body"} input={[input, setInput]} />
